...
export function useRawData() {
  ...
  async function fetchRawDataList() {
    ...
    error.value = null;
    try {
      rawDataList.value = await get<LandingPageRawData[]>("/landing-page-raw-data");
    } catch (err) {
      if (isAxiosError(err)) {
        if (err.response) {
        ...
        error.value = err instanceof Error ? err.message : "Error desconocido al obtener raw data";
      }
      rawDataList.value = [];
    } finally {
      loading.value = false;
  ...
  async function fetchLatestRawData() {
    ...
    error.value = null;
    try {
      latestRawData.value = await get<LandingPageRawData>("/landing-page-raw-data/latest");
    } catch (err) {
      if (isAxiosError(err)) {
        if (err.response) {
          const responseData = err.response.data as { detail?: string; message?: string } | undefined;
          ...
          error.value = err.message || "Error al obtener el raw data más reciente";
        }
      } else {
        error.value = err instanceof Error ? err.message : "Error desconocido al obtener raw data";
      }
  ...
  async function fetchRawDataById(id: string) {
    ...
    error.value = null;
    try {
      currentRawData.value = await get<LandingPageRawData>(`/landing-page-raw-data/${id}`);
    } catch (err) {
      if (isAxiosError(err)) {
        if (err.response) {
          const responseData = err.response.data as { detail?: string; message?: string } | undefined;
